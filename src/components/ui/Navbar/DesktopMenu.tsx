  ...
  return (
    ...
    >
      {!hasSubMenu && menu.href ? (
        <Link href={menu.href}>
          <span className="flex items-center text-foreground font-rajdhani font-semibold text-xs xl:uppercase lg:text-sm xl:text-lg whitespace-nowrap gap-1 hover:text-primary cursor-pointer px-3 py-1 rounded-xl transition-all duration-300">
            {menu.name}
          </span>
        </Link>
      ) : (
        <span className="flex items-center font-rajdhani font-semibold text-xs lg:text-sm xl:text-lg xl:uppercase text-foreground gap-1 hover:text-primary cursor-pointer px-3 py-1 rounded-xl transition-all duration-300">
          {menu.name}
          {hasSubMenu && (
            <ChevronDown
                    ...
                    <div className="space-y-4 pt-2">
                      {groupedSubMenus[heading]?.map((submenu) => (
                        ...
                        >
                          ...
                          <div>
                            <h4 className="font-rajdhani font-semibold text-foreground group-hover:text-primary transition-colors duration-200">
                              {submenu.name}
                            </h4>
                            <p className="text-sm text-foreground/70 group-hover:text-foreground/90 transition-colors duration-200 font-inter">
                              {submenu.desc}
                            </p>
                          </div>
                        </Link>
